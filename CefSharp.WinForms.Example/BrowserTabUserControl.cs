...
namespace CefSharp.WinForms.Example
{
   public partial class BrowserTabUserControl : UserControl
   {
      ...
      private bool saveToDiskEnabled;
      private int saveInterval = 10000;
      private string savePath;
      #endregion

      #region properties
      public bool SaveToDiskEnabled
      {
         get { return saveToDiskEnabled; }
         set
         {
            saveToDiskEnabled = value;
      ...
      public async void SaveSource(string path)
      {
         var htmlSource = await Browser.GetSourceAsync();
         try
         {
            //File.WriteAllText(path, htmlSource);

            AgilityPack.HtmlLoader loader = new AgilityPack.HtmlLoader();
            
            //loader.SaveInnerText(path);
            loader.GetData(htmlSource);
      ...
      private void WriteToDisk()
      {
         if(SaveToDiskEnabled)
         {
            ...
            {
               string path = Path.Combine(savePath, $"source_{DateTime.Now.ToString("yyyy-MM-dd-HH-mm-ss-fff")}");
               SaveSource(path);
